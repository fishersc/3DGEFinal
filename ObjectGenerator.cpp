...
ObjectGenerator::Vector3 ObjectGenerator::computeNormal(const Vector3& A, const Vector3& B, const Vector3& C)
{
...
ObjectGenerator::Vector3 ObjectGenerator::adjustNormal(const Vector3& A)
{
   glm::vec3 a(A.x, A.y, A.z);
...
float* ObjectGenerator::generateCuboid(float width, float height, float depth)
{
...
float* ObjectGenerator::generateLineCuboid(float width, float height, float depth)
{
   float w2 = width/2, h2 = height/2, d2 = depth/2;
   ...
   // There are 4 faces with 4 lines each.  Each line has 2 vertices with 8 components. 
   // Plus the first position is used to store the number of elements.
   // (4 x 4 x 2 x 8) + 1 = 257
   float* data = new float[257];
...
float* ObjectGenerator::generateCylinder(float radius, float height, int slices, int stacks)
{
   float rho = radius, theta = 0;
   float stepY = height / stacks;
   float stepX = 360.0f / slices;
   // Each facet has 2 triangles with 3 vertices each (2 x 3 = 6)
   int vertices = slices * stacks * 6; 
   // The top and bottom of the cylinder
   vertices += (slices * 6);
   // Each vertex has 11 components
   int numOfFloats = (vertices * 11); 

   float* data = new float[numOfFloats + 1]; // Add one to store the number of floats
   data[0] = (float)numOfFloats;

   float sinPhi = SIND(90);
   ...
   for(int stack = 0; stack < stacks; stack++){
      for(float theta = 0; theta < 360.0f; theta += stepX){
         // D   C
         // +---+
         // |\  |
         // | \ |
...
float* ObjectGenerator::generateSphere(float radius, int slices, int stacks)
{
...
float* ObjectGenerator::generateFlatSurface(float width, float depth, int rows, int cols)
{
   if(width <= 0) width = 1;
   if(depth <= 0) depth = 1;
   if(rows <= 0) rows = 1;
